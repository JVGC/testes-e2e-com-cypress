...
Cypress.Commands.add(
  'updateNote',
  (currentNoteDescription, newNoteDescription, attachFile = false) => {
    cy.intercept('GET', '**/notes/**').as('getNote')

    cy.contains('.list-group-item', currentNoteDescription).click()
    cy.wait('@getNote')

    cy.get('#content').clear().type(newNoteDescription)
    ...
    cy.contains('.list-group-item', currentNoteDescription).should('not.exist')
    cy.contains('.list-group-item', newNoteDescription).should('be.visible')
  }
)

Cypress.Commands.add('deleteNote', (noteDescription) => {
  cy.intercept('GET', '**/notes/**').as('getNote')
